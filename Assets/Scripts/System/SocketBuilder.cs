...
{
    [Header("生成設定")]
    [Tooltip("供隨機挑選的裝飾物 Prefab 清單（如冷氣、招牌等）")]
    public List<GameObject> propPrefabs;

    [Tooltip("每個掛點生成配件的機率 (0 = 不生成, 1 = 必定生成)")]
    [Range(0f, 1f)] 
    ...
    public void GenerateProps()
    {
        ...
        {
            Debug.LogError("[SocketBuilder] propPrefabs 清單為空，無法生成物件。");
            return;
        }
        ...
        foreach (Transform socket in sockets)
        {
            ...
            {
                // 隨機挑選其中一個 Prefab
                GameObject selectedPrefab = propPrefabs[Random.Range(0, propPrefabs.Count)];
